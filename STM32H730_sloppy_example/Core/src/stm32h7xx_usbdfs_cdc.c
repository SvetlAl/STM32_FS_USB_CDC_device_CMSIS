/****************************************************************
* STM32H7XX
* USB OTG HS FS device (CDC) implementation
*
...
static inline void set_FIFOs_sz(){
	USB1_OTG_HS->GRXFSIZ = RX_FIFO_SIZE; /* all EPs RX FIFO RAM size */
	USB1_OTG_HS->DIEPTXF0_HNPTXFSIZ = ((TX_EP0_FIFO_SIZE) << 16) | RX_FIFO_SIZE; /* EP0 TX FIFO RAM size */
	USB1_OTG_HS->DIEPTXF[0] = ((TX_EP1_FIFO_SIZE) << 16) | (RX_FIFO_SIZE+TX_EP0_FIFO_SIZE); /* EP1 TX FIFO RAM size */
...
* param
* retval
* A11 (DM), A12 (DP), A9(VBUS)
*/
void USB_OTG_HS_init_pinout(){
	#if defined(USB_PERIPH_FS_SPD) && defined(USB_PERIPH_USB1)
...
void USB_OTG_HS_init_device(){
	...
	USB1_OTG_HS->GCCFG = USB_OTG_GCCFG_PWRDWN | USB_OTG_GCCFG_VBDEN; /* Power up */
	
	//USB_OTG_DEVICE->DCTL = USB_OTG_DCTL_SDIS;  /* Soft disconnect */
	USB_OTG_PWRCLK->PCGCR |= USB_OTG_PCGCR_STPPCLK;
///	USB_OTG_PCGCCTL->PCGCCTL = 0;
	USB1_OTG_HS->GUSBCFG =  USB_OTG_GUSBCFG_FDMOD | USB_OTG_GUSBCFG_PHYSEL; /* Force device mode */
	USB1_OTG_HS->GUSBCFG &= ~(uint32_t)(0x0FUL << 10UL) ;  /* USB turnaround time (according to AHB and ReferenceManual) */
	USB1_OTG_HS->GUSBCFG |= (0x6 << 10);
	...
	USB1_OTG_HS->GINTSTS = 0xFFFFFFFF; /* Reset Global Interrupt status */
	USB_OTG_DEVICE->DCTL &= ~USB_OTG_DCTL_SDIS;   /* Soft connect */
	initEndPoints();
	

	NVIC_SetPriority(OTG_HS_IRQn, USB_FS_IRQ_P);
...
uint32_t USB_FlushTxFifo(uint32_t EPnum, uint32_t timeout){
	uint32_t count = 0;
	USB1_OTG_HS->GRSTCTL = (USB_OTG_GRSTCTL_TXFFLSH | (EPnum << 6));
	do{
		if (++count > timeout){
			return EP_FAILED;
		}
	}
	while ((USB1_OTG_HS->GRSTCTL & USB_OTG_GRSTCTL_TXFFLSH) == USB_OTG_GRSTCTL_TXFFLSH);
...
uint32_t USB_FlushRxFifo(uint32_t timeout){
	...
	while ((USB1_OTG_HS->GRSTCTL & USB_OTG_GRSTCTL_RXFFLSH) == USB_OTG_GRSTCTL_RXFFLSH);
...
void OTG_HS_IRQHandler(){
	...
	/**************************************************************/
	if(USB1_OTG_HS->GINTSTS & USB_OTG_GINTSTS_SOF){ 
	...
	/****************** USBRST Reset event ************************/
	/**************************************************************/
	if(USB1_OTG_HS->GINTSTS & USB_OTG_GINTSTS_USBRST){  
	...
	if(USB1_OTG_HS->GINTSTS & USB_OTG_GINTSTS_IEPINT){  /* IN endpoint event */	
	...
	if(USB1_OTG_HS->GINTSTS & USB_OTG_GINTSTS_OEPINT){ /* OUT endpoint event */
	...
	/********************* RXFLVL event ****************************/
	/***************************************************************/  
	if(USB1_OTG_HS->GINTSTS & USB_OTG_GINTSTS_RXFLVL){ /* RX fifo not empty event */
		uint32_t temp = USB1_OTG_HS->GRXSTSP;   /* Read out PacKeT STatuS */ 
