...
#define RX_BUFFER_EP0_SIZE 8 /* Enough to set linecoding */
#define RX_BUFFER_EP1_SIZE 128
...
eDeviceState device_state = 	DEVICE_STATE_DEFAULT;
EndPointStruct			EndPoint[EP_COUNT];
...
uint8_t circBufferTx[CIRC_BUFFER_TX_SIZE];
uint16_t writePtrTxCbuf =			0;
uint16_t readPtrTxCbuf =			0;
...
*/
circBufferAddress read_circBuffer(uint16_t len){
...
/**
* brief  Init GPIO to use USB OTG for stm32f105/107
* param
* param
* retval
* A11 and A12
...
void read_Fifo(uint8_t dfifo, uint16_t len){
	...
	for (uint32_t i = 0; i < block_cnt; i++){
		*(uint32_t *)EndPoint[dfifo].rxBuffer_ptr = USB_OTG_DFIFO(0);
...
uint32_t write_Fifo(uint8_t dfifo, uint8_t *src, uint16_t len){ 
	...
	for (uint32_t i = 0; (i < block_cnt) ; i++){
		USB_OTG_DFIFO(dfifo) = *((uint32_t *)src);
		src+=4;	
	} 
...
uint32_t USB_CDC_transferRXCallback_EP1(){
		...
		case REQ_TYPE_HOST_TO_DEVICE_GET_DEVICE_DECRIPTOR:
			switch(setup_pkt_data.setup_pkt.wValue){
				...
				case DESCRIPTOR_TYPE_CONFIGURATION:   						/* Request 0x0680  Value 0x0200 */
					...
					dest = (uint8_t *)configurationDescriptor;
					break;    
				case DESCRIPTOR_TYPE_DEVICE_QUALIFIER:  					/* Request 0x0680  Value 0x0600 */
					...
					dest = (uint8_t *)deviceQualifierDescriptor;  
				...
				case DESCRIPTOR_TYPE_PROD_STRING: 						/* Request 0x0680  Value 0x0302 */
					...
					dest = (uint8_t *)productStringDescriptor;          
					break;                     
				case DESCRIPTOR_TYPE_SERIAL_STRING: 						/* Request 0x0680  Value 0x0303 */
					...
					dest = (uint8_t *)serialNumberStringDescriptor;                              
					break;
				case DESCRIPTOR_TYPE_CONFIGURATION_STRING:					/* Request 0x0680  Value 0x0304 */
					...
					dest = (uint8_t *)configurationStringDescriptor;
					break;
...
uint32_t USB_CDC_transmit_scheduler(){
	...
	if(get_circBuffer_freeSize(0) < (512)) {
		USB_MASK_INTERRUPT(USB_OTG_GINTSTS_RXFLVL); // CHK
		toggle_Rx_EP_Status(1, EP_BUSY);
	}
	else if(get_circBuffer_freeSize(0) > (512)){
		USB_UNMASK_INTERRUPT(USB_OTG_GINTSTS_RXFLVL); // CHK
		toggle_Rx_EP_Status(1, EP_READY);
	}
	
	if((is_tx_ep_fifo_ready(1, 1) == EP_OK) & (is_circBuffer_empty(0) !=0 )){
